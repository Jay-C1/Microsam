...
def _sync_embedding_widget(widget, model_type, save_path, checkpoint_path, device, tile_shape, halo):
    ...
    model_family = "Natural Images (SAM)"  # Default family
    ...
    # Update the index for model size, e.g., 'base', 'tiny', etc.
    size_map = {"t": "tiny", "b": "base", "l": "large", "h": "huge"}
    model_size = size_map.get(model_type[4], "base")
    index = widget.model_size_dropdown.findText(model_size)
    if index > 0:
        widget.model_size_dropdown.setCurrentIndex(index)

    # Update the save path if given
    if save_path is not None and isinstance(save_path, str):
        widget.embeddings_save_path_param.setText(str(save_path))

    # Update the custom weights path if given
    if checkpoint_path is not None:
        widget.custom_weights_param.setText(str(checkpoint_path))

    # Set default device without accessing UI
    if device is not None:
        widget.device = device  # No dropdown used

    # Set default tile shape without accessing UI
    if tile_shape is not None:
        widget.tile_x, widget.tile_y = tile_shape

    # Set default halo without accessing UI
    if halo is not None:
        widget.halo_x, widget.halo_y = halo
