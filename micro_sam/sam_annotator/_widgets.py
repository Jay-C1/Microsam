...
def segment(viewer: "napari.viewer.Viewer", batched: bool = False) -> None:
    ...
    def segment_with_prompt(viewer: "napari.viewer.Viewer") -> Container:
        ...
        points_layer = viewer.layers.get("point_prompts")
        if points_layer is None:
            raise ValueError("point_prompts layer not found")

        # prompt 
        prompt_menu = create_prompt_menu(points_layer, ["positive", "negative"])

        # segment
        segment_widget = segment(viewer=viewer)

        # 
        container = Container(widgets=[prompt_menu, segment_widget])
        return container
...
class EmbeddingWidget(_WidgetBase):
    ...
    def _create_settings_widget(self):
        ...
        # Set default device (used internally, no UI shown)
        self.device = "auto"

        # Create UI for the embeddings save path
        ...
        # Create UI for custom weights path
        ...
        # Set default tile and halo values (used internally, no UI shown)
        self.tile_x, self.tile_y = 8, 8
        self.halo_x, self.halo_y = 32, 32

        # Create UI for the automatic segmentation mode
        ...
        # Wrap all settings in a collapsible container
